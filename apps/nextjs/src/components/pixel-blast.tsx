...
type PixelBlastProps = {
	...
	/** Shrinks each shape inside its cell; < 1 leaves a gap between neighbours. */
	shapeScale?: number;
...
float maskCircle(vec2 p, float cov){
  float r = sqrt(cov) * .25 * uShapeScale;
...
float maskDiamond(vec2 p, float cov){
  float r = sqrt(cov) * 0.564 * uShapeScale;
...
const PixelBlast: React.FC<PixelBlastProps> = ({
	...
	useEffect(() => {
		...
		if (!container) return;
		// Reduced motion: one static frame, no ripples, no liquid distortion.
		const reduced = window.matchMedia(
			"(prefers-reduced-motion: reduce)",
		).matches;
		const liquidOn = liquid && !reduced;
		const ripplesOn = enableRipples && !reduced;
		...
		const cfg: ReinitConfig = { antialias, liquid: liquidOn, noiseAmount };
		...
		if (mustReinit) {
			...
			const uniforms = {
				...
				uEnableRipples: { value: ripplesOn ? 1 : 0 },
			...
			// Reduced motion draws on demand (mount + resize) instead of per frame.
			...
			const animate = () => {
				...
				clock.update();
				if (!reduced && autoPauseOffscreen && !visibilityRef.current.visible)
					return;
				uniforms.uTime.value = reduced
					? timeOffset
					: timeOffset + clock.getElapsed() * speedRef.current;
				if (liquidEffect) {
		...
		t.uniforms.uShapeScale.value = shapeScale;
		t.uniforms.uEnableRipples.value = ripplesOn ? 1 : 0;
